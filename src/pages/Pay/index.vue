...
export default {
  ...
  methods: {
    async getPayMessage() {
      const result = await this.$API.reqPayMessage(this.orderId);
    ...
    async open() {
      ...
      this.$alert(`<img src=${url} />`, "微信支付", {
        ...
        showClose: false,
        //修改取消按钮的文本
        confirmButtonText: "支付成功点击跳转",
        //关闭弹出框的配置,第一个参数为点击按钮的类型，第二个参数为组件实例
      ...
      //需要不停的向后台发请求，查看是否支付成功
      //判断是否存在定时器，不存在就重新创建一个
      if (!this.timer) {
        this.timer = setInterval(async () => {
          ...
          if (result.code == 205) {
            ...
            clearInterval(this.timer);
            this.timer = null;
            //储存code,防止用户没付款时点击我已经付款了，用于后期判断
            this.code = result.code;
            //关闭弹出框
            this.$msgbox.close();
            //路由跳转
