        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="(spuSaleAttr, index) of spuSaleAttrList"
                :key="spuSaleAttr.id"
              >
                <dt class="title">{{ spuSaleAttr.saleAttrName }}</dt>
                <!-- 利用排他思想，当前点击的属性值，和当前属性值的同级兄弟(一个数组)传递给点击事件 -->
                <dd
                  changepirce="0"
                  v-for="(
            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  @change="inputNum"
                />
                <a href="javascript:" class="plus" @click="skuAdd">+</a>
                <a href="javascript:" class="mins" @click="skuReduce">-</a>
              </div>
              <div class="add">
                <!--以前咱侧的路由挑转：从A路由跳转到B路由，这里在加入购物车，进行路由转之前，发请求 把你购买的产品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...
    //点击向服务器发送求，得到购物车的数据并进行路由跳转
    addShopCar() {
      //1，排发action，将产品加入购物车的信息添加到服务器,返回写入数据库成功，不需要存在仓库
      try {
        //成功的话进行路由跳转
        this.$store.dispatch("fetchAddShopCar", {
          skuId: this.$route.params.skuId,
          skuNum: this.skuNum,
        });
        //成功进行路由跳转，跳转到购物车添加成功界面，并携带商品信息
        this.$router.push({
          name: "addCart",
  ...
  mounted() {
    this.$store.dispatch("fetchDetail", this.$route.params.skuId);
  },
};
</script>
